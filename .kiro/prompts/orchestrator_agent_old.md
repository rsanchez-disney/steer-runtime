...
```json
{
  ...
  "content": {
    "subagents": [
      {
        "agent_name": "story_analyzer_agent",
        "query": "Analyze Jira story https://myjira.disney.com/browse/DPAY-14337. Validate completeness. Return JSON with: title, description, acceptance_criteria (array), story_type, priority, components (array), incomplete (boolean), missing_fields, issues, questions"
      }
    ]
  }
}
...
### 1.5 Check Story Completeness

**After receiving story analysis, check if `incomplete: true`**:

If story is incomplete:

```
⚠️  STORY INCOMPLETE - CANNOT PROCEED
...
Questions for Clarification:
  1. What format should the export be? (CSV, Excel, JSON?)
  2. What data should be exported?
  3. What should happen if export fails?
  4. Should there be a size limit?

Please update the Jira story with:
  • Clear acceptance criteria
  • Specific technical requirements
  • Complete description

Once updated, run: Implement https://myjira.disney.com/browse/DPAY-14337
```

**STOP WORKFLOW. Do not proceed to codebase exploration.**

If story is complete, continue to step 2.
