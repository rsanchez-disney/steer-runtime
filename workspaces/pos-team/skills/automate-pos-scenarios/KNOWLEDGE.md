---
title: POS test automation — deep knowledge reference
type: architecture-and-patterns
updated: 2026-07-24
---

# POS test automation — deep knowledge reference

Load this file when debugging receipt validation, working with payment flows, adding venue fields, understanding multi-remote architecture, or configuring parallel execution.
...
- Suite: `@DSP_GO_regression`, `@DSP_GO_smoke`, `@multiremote`, `@connect_regression`, `@services`, `@QA_regression`
- Patterns: `@US:*`, `@TC:*`, `@BUG*`, `@TASK*`
...
## Menu profile architecture

### Venue data structure (`data/config/menu_profile_latest.json`)

Each venue is a nested domain-grouped object keyed by `dsp_id`:
...
- `resolveVenueField(venueData, key)` maps field keys to nested paths
...
// Top-level fields work with direct access (type, location, name, id, address)
menuContent[venueId].type  // resolveVenueField checks top-level first
```

### Adding new venue fields

1. Add it to the appropriate nested group in the JSON (`items`, `modifiers`, `fees`, or top-level)
2. Add the key mapping in the corresponding resolver function (`resolveItemField` or `resolveModifierField`)
3. ESLint will show complexity warnings on resolvers — acceptable for mapping functions

---

## Checks page object architecture

### Domain managers

Check operations are split into focused domain managers under `src/pageobjects/dsp/`:

| File                       | Responsibility                         |
|----------------------------|----------------------------------------|
| `mobile.checks.page.js`   | Navigation, check lookup, delegation   |
| `mobile.split.page.js`    | Split check operations                 |
| `mobile.combine.page.js`  | Combine check operations               |
| `mobile.transfer.page.js` | Transfer check operations              |

### Delegation chain

```text
step definition → dashboardPage → checksManager → {splitManager | combineManager | transferManager}
```

- `mobile.checks.page.js` imports all 3 domain managers and delegates
- Domain managers receive `selectOrderOption` as a bound callback to avoid circular imports
- Step definitions call the top-level checksManager API — sub-managers are transparent

### Key methods by domain

- **Split**: `splitCheck`, `searchSplitOrder`, `splitOrderDisplayed`, `selectSplitCheck`
- **Combine**: `createOrderForCombine`, `combineChecks`, `initiateCombineProcess`, `selectChecksForCombine`, `confirmCombineOperation`, `handleBarTabCombine`, `handleRegularCombine`
- **Transfer**: `transferOrder`, `validateTransferredOrder`

---

## Checks feature folder structure

### Organization pattern

Feature files follow a domain-first structure. Each check domain owns all its scenarios in one top-level folder with an optional `/regression` subfolder for complex/config-dependent scenarios:

```text
src/features/components/checks/
├── split/                    ← Core split scenarios
...
├── combine/                  ← Core combine scenarios
...
├── transfer/                 ← Core transfer scenarios
...
├── check.details/            ← Table number, guest count, submit options, check size
│   └── regression/
...
- **Domain-first, not tier-first** — `split/regression/` not `regression/split/`. Working on a domain keeps everything in one folder.
- **check.lifecycle** — Groups tests that verify the check state machine (open → closed, cancel, recall, reprint).
- **Glob-safe** — Suite configs use `checks/**/*.feature` pattern, so folder restructuring is transparent.
- **No content coupling** — Features reference steps, not paths. Step definitions are unaffected by folder layout.
