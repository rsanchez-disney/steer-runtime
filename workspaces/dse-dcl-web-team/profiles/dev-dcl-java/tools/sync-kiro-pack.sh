...
# Run this from a DCL Java service root (e.g., dcl-apps-activity-svc).
# Expected sibling repos:
#   ../dcl-cruise-101-webapi   (NestJS BFF)
#   ../dcl-apps-checkout-ui    (Angular SPA)
#
# You can override by setting:
#   WEBAPI_REPO=/path/to/dcl-cruise-101-webapi
#   UI_REPO=/path/to/dcl-apps-checkout-ui

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PACK_DIR="${ROOT}/_kiro-pack"

BACKEND_REPO="${ROOT}"
WEBAPI_REPO="${WEBAPI_REPO:-$(cd "${ROOT}/.." && pwd)/dcl-cruise-101-webapi}"
UI_REPO="${UI_REPO:-$(cd "${ROOT}/.." && pwd)/dcl-apps-checkout-ui}"
