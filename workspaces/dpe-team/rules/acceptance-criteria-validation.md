...
2. **Linked wiki pages** — Confluence/MyWiki pages linked from the Jira ticket (use `@mywiki/*` MCP tools to fetch linked page content)
3. **PR description or user input** — provided directly by the developer

If the ticket contains links to wiki pages (e.g., `mywiki.disney.com/...`), fetch those pages and extract ACs from them. Wiki pages often contain detailed requirements, acceptance criteria tables, or BDD scenarios that are not duplicated in the ticket itself.
