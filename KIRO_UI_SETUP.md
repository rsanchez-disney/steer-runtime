...
cp -r ~/steer-runtime/.kiro .kiro-steer

# Update resource paths
find .kiro-steer/agents -name "*.json" -exec sed -i '' 's|file://~/\.kiro/|file://.kiro-steer/|g' {} \;
...
```bash
cd ~/my-project
ln -s ~/steer-runtime/.kiro .kiro-steer
...
**For UI testing**: Copy agents to project (`.kiro-steer/`) with updated paths
