...
var SharePointApiClient = class {
  ...
  getSiteUrl() {
    this.loadEnv();
    if (!this.siteUrl)
      throw new Error("SHAREPOINT_SITE_URL is required.");
    return this.siteUrl;
  }
...
var apiClient = new SharePointApiClient();

// build/tools/listSites.js
var listSitesSchema = {
...
async function handleListSites(args) {
  try {
    ...
    const sites = (data.value || []).map((s) => `- **${s.displayName}** \u2014 ${s.webUrl} (id: ${s.id})`);
...
// build/tools/listDrives.js
var listDrivesSchema = {
...
async function handleListDrives(args) {
  try {
    const siteId = args.siteId || await apiClient.getSiteId();
    const data = await apiClient.graphRequest(`/sites/${siteId}/drives`);
    const drives = (data.value || []).map((d) => `- **${d.name}** \u2014 ${d.webUrl} (id: ${d.id})`);
...
async function handleListItems(args) {
  try {
    const { driveId, path } = args;
    const endpoint = path ? `/drives/${driveId}/root:/${path.split("/").map(encodeURIComponent).join("/")}:/children` : `/drives/${driveId}/root/children`;
    const data = await apiClient.graphRequest(endpoint);
    const items = (data.value || []).map((i) => {
      const type = i.folder ? "\u{1F4C1}" : "\u{1F4C4}";
...
// build/tools/searchDocuments.js
...
async function handleUploadDocument(args) {
  try {
    const { driveId, remotePath, localPath } = args;
    if ((0, import_path2.isAbsolute)(localPath) || localPath.includes("..")) {
      return { content: [{ type: "text", text: "Error: localPath must be a relative path without '..' traversals" }], isError: true };
    }
    const resolved = (0, import_path2.resolve)(process.cwd(), localPath);
    if (!resolved.startsWith(process.cwd())) {
      return { content: [{ type: "text", text: "Error: localPath resolves outside working directory" }], isError: true };
    }
    const fileBuffer = await (0, import_promises.readFile)(resolved);
    const encodedPath = remotePath.split("/").map(encodeURIComponent).join("/");
    const data = await apiClient.graphRequest(`/drives/${driveId}/root:/${encodedPath}:/content`, { method: "PUT", body: fileBuffer, headers: { "Content-Type": "application/octet-stream" } });
...
// build/index.js
var INSTANCE_PREFIX = process.env.SHAREPOINT_INSTANCE_PREFIX || "";
...
var SharePointMCPServer = class {
  server;
  constructor() {
    this.server = new Server({ name: INSTANCE_PREFIX ? `sharepoint-${INSTANCE_PREFIX.replace(/_$/, "")}` : "sharepoint-mcp", version: "0.1.0" }, { capabilities: { tools: {} } });
...
var server = new SharePointMCPServer();
