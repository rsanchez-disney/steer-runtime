...
async function handleNavigate(args) {
  try {
    ...
    await page2.goto(url, { waitUntil, timeout: 3e4 });
...
async function handleClick(args) {
  try {
    const { selector } = args;
    const page2 = await getPage();
    await page2.waitForSelector(selector, { timeout: 5e3 });
...
async function handleType(args) {
  try {
    ...
    await page2.waitForSelector(selector, { timeout: 5e3 });
    if (clear) {
      await page2.click(selector, { count: 3 });
      await page2.keyboard.press("Backspace");
    }
    await page2.type(selector, text);
...
async function handleGetText(args) {
  try {
    ...
    const page2 = await getPage();
    const text = await page2.$eval(selector, (el) => el.innerText);
    const truncated = text.length > 1e4 ? text.slice(0, 1e4) + "\n\n[... truncated at 10000 chars]" : text;
...
async function handleGetDom(args) {
  try {
    const { selector, outer = false } = args;
    const page2 = await getPage();
    let html;
    if (selector) {
      html = await page2.$eval(selector, (el, o) => o ? el.outerHTML : el.innerHTML, outer);
    } else {
      html = await page2.content();
    }
    const truncated = html.length > 2e4 ? html.slice(0, 2e4) + "\n\n[... truncated at 20000 chars]" : html;
    return { content: [{ type: "text", text: truncated }] };
  } catch (error) {
...
// build/tools/waitForSelector.js
...
async function handleWaitForSelector(args) {
  try {
    const { selector, timeout = 1e4, visible = true } = args;
    const page2 = await getPage();
    await page2.waitForSelector(selector, { timeout, visible });
...
var ChromeMCPServer = class {
  server;
  constructor() {
    this.server = new Server({ name: "chrome-mcp", version: "0.1.0" }, { capabilities: { tools: {} } });
...
var server = new ChromeMCPServer();
