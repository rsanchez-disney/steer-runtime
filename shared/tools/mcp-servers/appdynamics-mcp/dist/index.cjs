...
var AppDynamicsApiClient = class {
  controllerUrl = null;
  clientId = null;
  clientSecret = null;
  ...
  async validate() {
    const missing = [];
    if (!process.env.APPD_CONTROLLER_URL)
      missing.push("APPD_CONTROLLER_URL");
    if (!process.env.APPD_CLIENT_ID)
      missing.push("APPD_CLIENT_ID");
    if (!process.env.APPD_CLIENT_SECRET)
      missing.push("APPD_CLIENT_SECRET");
    if (missing.length) {
...
var apiClient = new AppDynamicsApiClient();

// src/tools/listApplications.ts
var listApplicationsSchema = {
...
async function handleListApplications() {
  ...
  const apps = Array.isArray(data) ? data.map((a) => ({ id: a.id, name: a.name })) : data;
  return {
    content: [{ type: "text", text: JSON.stringify(apps, null, 2) }]
  };
}

// src/tools/getApplicationHealth.ts
var getApplicationHealthSchema = {
  name: "get_application_health",
  description: "Get the overall health summary for an application. Returns health rule violations in the last hour.",
  inputSchema: {
    type: "object",
    properties: {
      appName: {
...
async function handleGetApplicationHealth(args) {
  const { appName } = args;
  const now = Date.now();
...
// src/tools/getBusinessTransactions.ts
var getBusinessTransactionsSchema = {
...
async function handleGetBusinessTransactions(args) {
  const { appName } = args;
  const data = await apiClient.restGet(`/applications/${appName}/business-transactions`);
  const bts = Array.isArray(data) ? data.map((bt) => ({
    id: bt.id,
    name: bt.name,
...
// src/tools/getMetricData.ts
var getMetricDataSchema = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      metricPath: {
        type: "string",
        description: 'The full metric path (e.g. "Overall Application Performance|Average Response Time (ms)")'
      },
      durationMinutes: {
        type: "number",
...
async function handleGetMetricData(args) {
...
async function handleGetTiers(args) {
  const { appName } = args;
  const data = await apiClient.restGet(`/applications/${appName}/tiers`);
  const tiers = Array.isArray(data) ? data.map((t2) => ({
    id: t2.id,
    name: t2.name,
    numberOfNodes: t2.numberOfNodes
  })) : data;
  return {
...
async function handleGetNodes(args) {
  ...
  const path = tierName ? `/applications/${appName}/tiers/${tierName}/nodes` : `/applications/${appName}/nodes`;
  const data = await apiClient.restGet(path);
  const nodes = Array.isArray(data) ? data.map((n) => ({
    id: n.id,
    name: n.name,
    tierName: n.tierName,
...
// src/tools/getHealthViolations.ts
var getHealthViolationsSchema = {
...
async function handleGetHealthViolations(args) {
  const { appName, durationMinutes = 60 } = args;
  const now = Date.now();
  const start = now - durationMinutes * 60 * 1e3;
...
// src/tools/getErrorRate.ts
var getErrorRateSchema = {
  name: "get_error_rate",
...
async function handleGetErrorRate(args) {
  const { appName, durationMinutes = 60 } = args;
  const params = {
    "time-range-type": "BEFORE_NOW",
    "duration-in-mins": String(durationMinutes),
...
// src/tools/getSnapshots.ts
var getSnapshotsSchema = {
  name: "get_snapshots",
...
async function handleGetSnapshots(args) {
  ...
  if (Array.isArray(data)) {
    const snaps = data.map((s2) => ({
      id: s2.id,
      businessTransactionName: s2.businessTransactionName,
      URL: s2.URL,
      timeTakenInMilliSecs: s2.timeTakenInMilliSecs,
      errorOccurred: s2.errorOccurred,
      userExperience: s2.userExperience
    }));
    return {
      content: [
        {
...
// src/tools/getAnomalies.ts
var getAnomaliesSchema = {
  name: "get_anomalies",
  description: "Get anomaly violations detected by AppDynamics AI for an application, including suspected root causes. Requires controller version 26.x+.",
  inputSchema: {
    type: "object",
    properties: {
      appName: {
...
async function handleGetAnomalies(args) {
            ...
            text: JSON.stringify(
              {
                ...
                message: "The Anomaly Detection API endpoint returned 404. This typically means anomaly detection is not enabled on this controller or the controller version does not support this API."
              },
              null,
              2
            )
  ...
  const anomalies = violations.map((v) => {
    const anomaly = {
      id: v.id,
      status: v.status,
    ...
    if (includeSuspectedCauses && v.eventDetailMap) {
      anomaly.events = Object.entries(v.eventDetailMap).map(
        ([eventId, event]) => ({
          ...
          suspectedCauses: (event.suspectedCauses ?? []).map((cause) => ({
  ...
  const active = anomalies.filter((a) => a.status === "OPEN");
  const resolved = anomalies.filter((a) => a.status === "RESOLVED");
  const withCauses = anomalies.filter(
    (a) => a.events?.some((e2) => e2.suspectedCauses?.length > 0)
  );
  const result = {
    application: appName,
    app_id: appId,
...
// src/tools/getBackends.ts
var getBackendsSchema = {
  name: "get_backends",
...
async function handleGetBackends(args) {
  const { appName } = args;
  const data = await apiClient.restGet(`/applications/${appName}/backends`);
  const backends = Array.isArray(data) ? data.map((b) => ({
    id: b.id,
    name: b.name,
...
async function handleGetErrors(args) {
  const { appName, durationMinutes = 60 } = args;
  const data = await apiClient.restGet(`/applications/${appName}/metric-data`, {
  ...
  if (Array.isArray(data)) {
    const errors = data.filter((m2) => m2.metricValues?.length > 0 && m2.metricValues[0].sum > 0).map((m2) => ({
      metricPath: m2.metricPath,
      errorName: m2.metricPath?.split("|")?.[1] || "unknown",
      totalErrors: m2.metricValues[0].sum,
      errorsPerMinute: m2.metricValues[0].value,
      maxPerMinute: m2.metricValues[0].max
...
async function handleGetEvents(args) {
  ...
  if (Array.isArray(data)) {
    const events = data.map((e2) => ({
      ...
      affectedEntities: e2.affectedEntities?.map((ae) => ({
        name: ae.name,
        type: ae.entityType
      }))
    }));
...
// src/tools/getPolicies.ts
var getPoliciesSchema = {
  name: "get_policies",
...
async function handleGetPolicies(args) {
  const { appName } = args;
  const appId = await apiClient.resolveAppId(appName);
...
// src/tools/getActions.ts
var getActionsSchema = {
  name: "get_actions",
...
async function handleGetActions(args) {
  const { appName } = args;
  const appId = await apiClient.resolveAppId(appName);
  const data = await apiClient.rawGet(
...
// src/tools/getDashboards.ts
var getDashboardsSchema = {
  name: "get_dashboards",
...
// src/tools/compareMetrics.ts
var compareMetricsSchema = {
...
async function handleCompareMetrics(args) {
  ...
  const now = Date.now();
  const baselineEnd = now - baselineOffsetMinutes * 60 * 1e3;
  const baselineStart = baselineEnd - baselineDurationMinutes * 60 * 1e3;
...
var AppDynamicsMCPServer = class {
  server;
  constructor() {
...
var server = new AppDynamicsMCPServer();
