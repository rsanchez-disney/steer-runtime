...
var figma = new FigmaApiClient();

// src/tools/getFile.ts
var getFileSchema = {
  name: "get_figma_file",
...
function summarizeNode(node, indent = "") {
  ...
  if (node.absoluteBoundingBox) {
    ...
    out += ` [${Math.round(b.width)}\xD7${Math.round(b.height)}]`;
  }
  if (node.fills?.length) out += ` fills:${node.fills.length}`;
  if (node.children?.length) {
    out += `
${node.children.map((c) => summarizeNode(c, indent + "  ")).join("\n")}`;
  }
  return out;
}
async function handleGetNode(args) {
  ...
  const nodes = Object.entries(data.nodes || {}).map(
    ([id, val]) => val?.document ? summarizeNode(val.document) : `${id}: not found`
  );
  return { content: [{ type: "text", text: nodes.join("\n\n") }] };
}

// src/tools/getComments.ts
var getCommentsSchema = {
...
async function handleGetComments(args) {
  ...
  const comments = (data.comments || []).map(
    (c) => `[${c.resolved_at ? "\u2705" : "\u{1F4AC}"}] ${c.user?.handle || "unknown"} (${c.created_at?.slice(0, 10)}):
  ${c.message}`
  );
  return {
    content: [{
...
async function handleGetStyles(args) {
  ...
  const styles = (data.meta?.styles || []).map(
    (s) => `  \u2022 ${s.name} (${s.style_type}) \u2014 key: ${s.key}`
  );
  return {
    content: [{
      type: "text",
...
// src/tools/exportImages.ts
var exportImagesSchema = {
...
async function handleExportImages(args) {
