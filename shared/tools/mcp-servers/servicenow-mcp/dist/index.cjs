...
var ServiceNowApiClient = class {
  ...
  async getSysId(table, numberField, numberValue) {
  ...
  async getSysIdByName(table, name) {
    const result = await this.get(`/table/${table}`, {
  ...
  async getUserSysIdByEmail(email) {
  ...
  async validate() {
    ...
    if (!process.env.SNOW_INSTANCE)
      missing.push("SNOW_INSTANCE");
    if (!process.env.SNOW_API_USERNAME)
      missing.push("SNOW_API_USERNAME");
    if (!process.env.SNOW_API_PASSWORD)
      missing.push("SNOW_API_PASSWORD");
    if (missing.length) {
...
var apiClient = new ServiceNowApiClient();

// src/tools/getIncident.ts
var getIncidentSchema = {
  name: "get_incident",
...
async function handleGetIncident(args) {
  ...
  const fields = {
    number: r2.number,
    short_description: r2.short_description,
    state: r2.state,
    priority: r2.priority,
    assignment_group: r2.assignment_group?.display_value ?? "",
    assigned_to: r2.assigned_to?.display_value ?? "",
    cmdb_ci: r2.cmdb_ci?.display_value ?? "",
    parent_incident: r2.parent_incident?.display_value ?? "",
    sys_created_on: r2.sys_created_on
  };
  return { content: [{ type: "text", text: JSON.stringify(fields, null, 2) }] };
}

// src/tools/addWorkNote.ts
var addWorkNoteSchema = {
  name: "add_work_note",
...
async function handleAddWorkNote(args) {
...
async function handleChangeCi(args) {
...
// src/tools/changeAssignmentGroup.ts
var changeAssignmentGroupSchema = {
...
async function handleChangeAssignmentGroup(args) {
  ...
  if (assignedToEmail) {
    ...
    if (userSysId)
      updateData.assigned_to = userSysId;
  }
...
// src/tools/addParentIncident.ts
var addParentIncidentSchema = {
...
async function handleAddParentIncident(args) {
...
// src/tools/resolveIncident.ts
var resolveIncidentSchema = {
...
async function handleResolveIncident(args) {
  const {
    incidentNumber,
    closeNotes,
...
async function handleUpdateIncident(args) {
...
// src/tools/queryIncidents.ts
var queryIncidentsSchema = {
...
async function handleQueryIncidents(args) {
  const {
    query,
...
async function handleCreateIncident(args) {
  const {
    shortDescription,
    description,
    assignmentGroup,
  ...
  const data = { short_description: shortDescription, priority };
  if (description)
...
// src/tools/createProblem.ts
var createProblemSchema = {
  name: "create_problem",
...
async function handleCreateProblem(args) {
...
// src/tools/createChangeRequest.ts
var createChangeRequestSchema = {
...
async function handleCreateChangeRequest(args) {
  const {
    shortDescription,
    description,
    assignmentGroup,
  ...
  const typeMap = { normal: "normal", standard: "standard", emergency: "emergency" };
  const data = {
...
async function handleGetCtask(args) {
  ...
  const fields = {
    ...
    assignment_group: r2.assignment_group?.display_value ?? "",
    assigned_to: r2.assigned_to?.display_value ?? "",
    change_request: r2.change_request?.display_value ?? "",
    sys_created_on: r2.sys_created_on
  };
  return { content: [{ type: "text", text: JSON.stringify(fields, null, 2) }] };
}

// src/tools/addCtaskWorkNote.ts
var addCtaskWorkNoteSchema = {
...
async function handleAddCtaskWorkNote(args) {
...
// src/tools/updateCtask.ts
var updateCtaskSchema = {
  name: "update_ctask",
...
async function handleUpdateCtask(args) {
...
// src/tools/closeCtask.ts
var closeCtaskSchema = {
  name: "close_ctask",
...
var CLOSED_VALUES = /* @__PURE__ */ new Set(["3", "4", "7", "8"]);
var closeCodeLabels = {
...
async function handleCloseCtask(args) {
  ...
  if (parentChgSysId) {
    ...
    if (lockedStates.some((s2) => parentState.toLowerCase().includes(s2))) {
      return {
        content: [{ type: "text", text: JSON.stringify({
          status: "blocked",
          ctask: ctaskNumber,
...
// src/tools/getIncidentComments.ts
var getIncidentCommentsSchema = {
...
async function handleGetIncidentComments(args) {
  ...
  const entries = (result?.result ?? []).map((e2) => ({
    type: e2.element?.display_value || e2.element?.value,
    createdOn: e2.sys_created_on?.display_value || e2.sys_created_on?.value,
    createdBy: e2.sys_created_by?.display_value || e2.sys_created_by?.value,
    text: e2.value?.display_value || e2.value?.value
  }));
  return {
...
// src/tools/getRelatedIncidents.ts
var getRelatedIncidentsSchema = {
...
async function handleGetRelatedIncidents(args) {
  ...
  if (ciSysId) {
    ...
    for (const r2 of ciResult?.result ?? []) {
      related.push({
        number: r2.number?.display_value || r2.number?.value,
        relationship: "same_ci",
        shortDescription: r2.short_description?.display_value || r2.short_description?.value,
        state: r2.state?.display_value || r2.state?.value,
        priority: r2.priority?.display_value || r2.priority?.value
      });
    }
  }
  if (parentSysId) {
    ...
    for (const r2 of childResult?.result ?? []) {
      related.push({
        number: r2.number?.display_value || r2.number?.value,
...
// src/tools/getChangeRequest.ts
var getChangeRequestSchema = {
...
async function handleGetChangeRequest(args) {
  ...
  const fields = {
    ...
    close_code: r2.close_code,
    close_notes: r2.close_notes,
    sys_created_on: r2.sys_created_on
  };
  return { content: [{ type: "text", text: JSON.stringify(fields, null, 2) }] };
}

// src/tools/getCiDetails.ts
var getCiDetailsSchema = {
  name: "get_ci_details",
...
async function handleGetCiDetails(args) {
  const { ciName } = args;
...
async function handleSearchKnowledgeBase(args) {
  ...
  const articles = (result?.result ?? []).map((a) => ({
    number: a.number,
...
async function handleGetOnCall(args) {
  ...
  const members = (result?.result ?? []).map((m2) => ({
    member: m2.member?.display_value ?? m2.member,
    roster: m2.roster?.display_value ?? m2.roster,
    order: m2.order
  }));
...
// src/tools/bulkUpdateIncidents.ts
var bulkUpdateIncidentsSchema = {
...
async function handleBulkUpdateIncidents(args) {
  ...
  const numbers = incidentNumbers.split(",").map((n) => n.trim()).filter(Boolean);
  const results = [];
  ...
  const successCount = results.filter((r2) => r2.status === "success").length;
  return {
    content: [{ type: "text", text: JSON.stringify({
      status: "complete",
...
// src/tools/getIncidentTimeline.ts
var getIncidentTimelineSchema = {
...
async function handleGetIncidentTimeline(args) {
  ...
  const auditEntries = (auditResult?.result ?? []).map((e2) => ({
    timestamp: e2.sys_created_on?.display_value || e2.sys_created_on,
    field: e2.fieldname?.display_value || e2.fieldname,
    oldValue: e2.oldvalue?.display_value || e2.oldvalue,
    newValue: e2.newvalue?.display_value || e2.newvalue,
    user: e2.user?.display_value || e2.user,
    type: "field_change"
  }));
  ...
  const journalEntries = (journalResult?.result ?? []).map((e2) => ({
    timestamp: e2.sys_created_on,
    field: e2.element,
    newValue: (e2.value || "").substring(0, 500),
    user: e2.sys_created_by,
    type: e2.element === "work_notes" ? "work_note" : "comment"
  }));
  const timeline = [...auditEntries, ...journalEntries].sort((a, b) => (b.timestamp || "").localeCompare(a.timestamp || ""));
  return {
...
var ServiceNowMCPServer = class {
  ...
  setupToolHandlers() {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: tools.map((t2) => t2.schema)
    }));
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      const { name, arguments: args } = request.params;
      try {
        const tool = tools.find((t2) => t2.schema.name === name);
        if (!tool) {
...
var server = new ServiceNowMCPServer();
