...
import { authenticateViaBrowser, type TeamsTokens } from "./auth.js";
...
export class TeamsClient {
  ...
  private nativeHttp: AxiosInstance;
  private chatHttp: AxiosInstance;  // separate http client for chat/message operations
  private config: TeamsConfig;
  private accessToken: string | null = null;
  private chatToken: string | null = null;  // dedicated token for chatsvc endpoints
  private refreshToken: string | null = null;
  private skypeToken: string | null = null;
  private tokenExpiry: number = 0;
  private chatTokenExpiry: number = 0;
  private apiMode: ApiMode = "graph";
  private chatServiceUrl: string = "https://teams.microsoft.com/api/csa";

  // Pending device code flow state
  private pendingDeviceCode: string | null = null;
  private pendingPollInterval: number = 5;
  ...
  private loadCachedToken() {
    try {
      if (fs.existsSync(TOKEN_CACHE_FILE)) {
        const data: CachedToken = JSON.parse(fs.readFileSync(TOKEN_CACHE_FILE, "utf-8"));
  ...
  getApiMode(): ApiMode { return this.apiMode; }
  ...
  private getChatToken(): string {
    ...
    if (this.chatToken && Date.now() < this.chatTokenExpiry) return this.chatToken;
    if (this.accessToken && Date.now() < this.tokenExpiry) return this.accessToken;
    throw new Error("AUTH_REQUIRED: No chat token available. Set TEAMS_CHAT_TOKEN env var or use 'auth_login'.");
  }

  async setManualToken(token: string): Promise<{ mode: ApiMode; error?: string }> {
    const rawToken = token.replace(/^Bearer\s+/i, "");
    ...
    try {
      ...
      if (data?.tokens) {
        const graphEntry = Object.entries(data.tokens).find(
          ([_k, v]: [string, any]) => v?.tokenValue && (
            v?.resourceId?.includes("graph") || _k.toLowerCase().includes("graph")
          )
        );
        if (graphEntry) {
          this.accessToken = (graphEntry[1] as any).tokenValue;
          this.apiMode = "graph";
          this.saveCachedToken();
          return { mode: "graph" };
        }
  ...
  private async getToken(): Promise<string> {
    if (this.accessToken && Date.now() < this.tokenExpiry) return this.accessToken;
    if (this.config.authMode === "client_credentials") return this.getTokenClientCredentials();
    if (this.refreshToken) {
      try { return await this.getTokenRefresh(); } catch { /* fall through */ }
    }
  ...
  private async getTokenRefresh(): Promise<string> {
    const url = `https://login.microsoftonline.com/${this.config.tenantId}/oauth2/v2.0/token`;
    const res = await axios.post(url, new URLSearchParams({
  ...
  async startDeviceCodeFlow(): Promise<{ userCode: string; verificationUri: string; message: string }> {
  ...
  async pollDeviceCode(): Promise<{ authenticated: boolean; error?: string }> {
    if (!this.pendingDeviceCode) return { authenticated: false, error: "No pending flow." };
    ...
    for (let i = 0; i < 60; i++) {
      try {
        ...
        if (error === "authorization_pending") { await new Promise(r => setTimeout(r, this.pendingPollInterval * 1000)); continue; }
        if (error === "slow_down") { await new Promise(r => setTimeout(r, 10000)); continue; }
        this.pendingDeviceCode = null;
  ...
  async reauthenticateViaBrowser(): Promise<void> {
    if (this.reauthInProgress) return this.reauthInProgress;
   ...
   */
  async withAutoReauth<T>(fn: () => Promise<T>): Promise<T> {
    try {
      return await fn();
    } catch (err) {
  ...
  private async nativeGet(path: string, params?: Record<string, string>) {
    return this.nativeHttp.get(`${this.chatServiceUrl}${path}`, { params });
  }

  private async nativePost(path: string, body: unknown) {
  ...
  async getChannel(teamId: string, channelId: string) {
    ...
    if (Array.isArray(channels)) {
      return channels.find((c: any) => c.id === channelId || c.channelId === channelId) || null;
    }
    return null;
  }
  ...
  async listMessages(teamId: string, channelId: string, top: number = 20, before?: string) {
    return this.withAutoReauth(async () => {
    if (this.apiMode === "graph") {
      const params: Record<string, any> = { $top: top, $orderby: "lastModifiedDateTime desc" };
      if (before) {
    ...
    const nativeParams: Record<string, string> = { view: "msnp24Equivalent|supportsMessageProperties", pageSize: String(top) };
    if (before) {
      nativeParams.startTime = before;
    }
    const res = await this.chatHttp.get(
      `https://teams.microsoft.com/api/chatsvc/amer/v1/users/ME/conversations/${encodedChannelId}/messages`,
      { params: nativeParams },
    );
  ...
  async getMessage(teamId: string, channelId: string, messageId: string) {
    ...
    if (Array.isArray(messages)) {
      return messages.find((m: any) => m.id === messageId || m.messageId === messageId) || null;
    }
    return null;
  }
  ...
  async listReplies(teamId: string, channelId: string, messageId: string) {
    ...
    const res = await this.chatHttp.get(
      `https://teams.microsoft.com/api/chatsvc/amer/v1/users/ME/conversations/${conversationId}/messages`,
      { params: { view: "msnp24Equivalent|supportsMessageProperties", pageSize: "50" } },
    );
    return res.data?.messages || res.data?.value || res.data;
  }

  private buildNativeMessageBody(conversationId: string, content: string) {
  ...
  async sendMessage(teamId: string, channelId: string, content: string) {
    ...
    const res = await this.chatHttp.post(
      `https://teams.microsoft.com/api/chatsvc/amer/v1/users/ME/conversations/${encodedChannelId}/messages`,
      body,
    );
    return res.data;
  ...
  async replyToMessage(teamId: string, channelId: string, messageId: string, content: string) {
    if (this.apiMode === "graph") {
    ...
    const res = await this.chatHttp.post(
      `https://teams.microsoft.com/api/chatsvc/amer/v1/users/ME/conversations/${encodedThreadId}/messages`,
      body,
    );
    return res.data;
  }
  ...
  async listMembers(teamId: string) {
    ...
    const res = await this.nativeHttp.get(
      `https://teams.microsoft.com/api/csa/api/v1/teams/${teamId}/members`,
    );
  ...
  async listChatMessages(chatId: string, top: number = 20) {
    if (this.apiMode === "graph") {
    ...
    const res = await this.chatHttp.get(
      `https://teams.microsoft.com/api/chatsvc/amer/v1/users/ME/conversations/${encodedChatId}/messages`,
      { params: { view: "msnp24Equivalent|supportsMessageProperties", pageSize: String(top) } },
    );
    return res.data?.messages || res.data?.value || res.data;
  }
  ...
  async reactToMessage(conversationId: string, messageId: string, emoji: string) {
    ...
    const res = await this.chatHttp.put(
      `https://teams.microsoft.com/api/chatsvc/amer/v1/users/ME/conversations/${encodedConvId}/messages/${messageId}/properties?name=emotions`,
      { emotions: { key: emoji, value: Date.now() } },
    );
    return res.data;
  }
}
