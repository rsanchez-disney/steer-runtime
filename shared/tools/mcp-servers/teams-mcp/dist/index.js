...
// --- Auth ---
server.tool("auth_login", "Start Teams authentication. Returns a URL and code to enter in your browser. After authenticating, call auth_complete.", {}, async () => {
    try {
...
server.tool("auth_complete", "Complete Teams authentication after signing in via browser. Call this after using auth_login and completing the browser sign-in.", {}, async () => {
...
server.tool("auth_status", "Check if you are currently authenticated with Teams.", {}, async () => {
    const status = [];
    try {
...
server.tool("auth_token", "Authenticate by pasting a Bearer token from your Teams browser session. Open teams.microsoft.com → DevTools (F12) → Network → find a request to graph.microsoft.com → copy the Authorization header value.", { token: z.string().describe("Bearer token from browser (with or without 'Bearer ' prefix)") }, async (args) => {
    try {
...
server.tool("auth_reset", "Reset Teams authentication by clearing all cached tokens and browser state. Use this when authentication is broken or tokens have expired beyond recovery.", {}, async () => {
    try {
...
server.tool("list_channels", tools.list_channels.description, tools.list_channels.schema.shape, async (args) => {
    ...
    const compact = Array.isArray(result) ? result.map((c) => ({
        id: c.id || c.channelId,
...
server.tool("list_messages", tools.list_messages.description, tools.list_messages.schema.shape, async (args) => {
    ...
    const compact = Array.isArray(result) ? result.map((m) => ({
        id: m.id || m.messageId,
