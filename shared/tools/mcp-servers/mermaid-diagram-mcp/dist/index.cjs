...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "generate_mermaid_image") {
    const { content, inputPath, outputPath, width = 1600, height = 1200, scale = 2 } = request.params.arguments;
    try {
      ...
      const mmdcPath = (0, import_path.join)(__dirname, "../node_modules/.bin/mmdc");
      let inputFile;
      let shouldCleanup = false;
      if (inputPath) {
        if (!(0, import_fs.existsSync)(inputPath)) {
          throw new Error(`Input file not found: ${inputPath}`);
        }
        inputFile = inputPath;
      } else {
        inputFile = (0, import_path.join)(process.cwd(), "temp.mmd");
        (0, import_fs.writeFileSync)(inputFile, content);
        shouldCleanup = true;
      }
      ...
      if (shouldCleanup) {
        (0, import_child_process.execSync)(`rm "${inputFile}"`);
      }
      return {
        content: [
          {
