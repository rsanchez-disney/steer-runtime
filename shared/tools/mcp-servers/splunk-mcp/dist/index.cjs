...
var SplunkApiClient = class {
  ...
  async createSearch(query, params = {}) {
  ...
  async getSearchResults(sid, count = 100, offset = 0) {
    return this.get(`/services/search/jobs/${sid}/results`, {
      count: String(count),
      offset: String(offset)
    });
  }
  async waitForSearch(sid, maxWaitMs = 6e4) {
    ...
    while (Date.now() - start < maxWaitMs) {
      ...
      if (entry?.isDone)
        return true;
      if (entry?.isFailed)
        throw new Error(`Search failed: ${entry.messages?.[0]?.text || "unknown error"}`);
...
var apiClient = new SplunkApiClient();

// src/tools/searchEvents.ts
...
async function handleSearchEvents(args) {
  ...
  await apiClient.waitForSearch(sid, maxWaitSeconds * 1e3);
...
// src/tools/getIndexes.ts
var getIndexesSchema = {
  name: "get_indexes",
...
async function handleGetIndexes() {
  const data = await apiClient.get("/services/data/indexes", { count: "0" });
  const entries = data?.entry ?? [];
  const indexes = entries.map((e2) => ({
    name: e2.name,
    totalEventCount: e2.content?.totalEventCount,
    currentDBSizeMB: e2.content?.currentDBSizeMB,
    disabled: e2.content?.disabled
  }));
  return {
...
// src/tools/getSavedSearches.ts
var getSavedSearchesSchema = {
...
async function handleGetSavedSearches(args) {
  const { app = "search", count = 50 } = args;
  const data = await apiClient.get(`/servicesNS/-/${app}/saved/searches`, {
    count: String(count)
  });
  const entries = data?.entry ?? [];
  const searches = entries.map((e2) => ({
    name: e2.name,
    search: e2.content?.search,
    isScheduled: e2.content?.is_scheduled,
    nextScheduledTime: e2.content?.next_scheduled_time,
    disabled: e2.content?.disabled
  }));
  return {
...
// src/tools/runSavedSearch.ts
var runSavedSearchSchema = {
...
async function handleRunSavedSearch(args) {
...
// src/tools/getJobStatus.ts
var getJobStatusSchema = {
  name: "get_job_status",
...
async function handleGetJobStatus(args) {
  const { sid } = args;
...
async function handleGetAlerts(args) {
  ...
  const entries = data?.entry ?? [];
  const alerts = entries.map((e2) => ({
    name: e2.name,
    triggeredAlertCount: e2.content?.triggered_alert_count,
    severity: e2.content?.alert_severity,
    published: e2.published
  }));
  return {
...
// src/tools/getAlertDetails.ts
var getAlertDetailsSchema = {
...
async function handleGetAlertDetails(args) {
  ...
  const triggers = firedEntries.map((e2) => ({
    triggerTime: e2.published,
    sid: e2.content?.sid,
    resultCount: e2.content?.triggered_alerts,
    severity: e2.content?.severity
  }));
  return {
...
// src/tools/getAlertHistory.ts
var getAlertHistorySchema = {
...
async function handleGetAlertHistory(args) {
  ...
  const entries = data?.entry ?? [];
  const history = entries.map((e2) => ({
    triggerTime: e2.published,
    sid: e2.content?.sid,
    resultCount: e2.content?.triggered_alerts,
    severity: e2.content?.severity,
    expirationTime: e2.content?.expiration_time_rendered
  }));
  const totalTriggers = history.length;
  let avgIntervalMs = 0;
  if (history.length > 1) {
    const times = history.map((h2) => new Date(h2.triggerTime).getTime()).filter((t2) => !isNaN(t2)).sort((a, b) => b - a);
    if (times.length > 1) {
      const intervals = [];
      for (let i2 = 0; i2 < times.length - 1; i2++) {
        intervals.push(times[i2] - times[i2 + 1]);
      }
  ...
  const summary = {
    alertName,
    totalTriggers,
    avgIntervalMinutes: avgIntervalMs > 0 ? Math.round(avgIntervalMs / 6e4) : null,
...
// src/tools/suppressAlert.ts
var suppressAlertSchema = {
  name: "suppress_alert",
...
async function handleSuppressAlert(args) {
  ...
  return {
    content: [{ type: "text", text: JSON.stringify({
      ...
      message: action === "disable" ? `Alert '${alertName}' has been suppressed (disabled). Remember to re-enable after maintenance.` : `Alert '${alertName}' has been re-enabled.`
    }, null, 2) }]
  };
}

// src/tools/getServerInfo.ts
var getServerInfoSchema = {
...
// src/tools/oneshotSearch.ts
var oneshotSearchSchema = {
  name: "oneshot_search",
...
async function handleOneshotSearch(args) {
...
// src/tools/getSourcetypes.ts
var getSourcetypesSchema = {
...
async function handleGetSourcetypes(args) {
  ...
  if (index) {
    ...
    await apiClient.waitForSearch(sid, 3e4);
    const results = await apiClient.getSearchResults(sid, count);
    const sourcetypes2 = results?.results ?? [];
    return {
      content: [{ type: "text", text: JSON.stringify({ index, count: sourcetypes2.length, sourcetypes: sourcetypes2 }, null, 2) }]
    };
  }
  const data = await apiClient.get("/services/saved/sourcetypes", { count: String(count) });
  const entries = data?.entry ?? [];
  const sourcetypes = entries.map((e2) => ({
    name: e2.name,
    description: e2.content?.description || ""
  }));
  return {
...
async function handleGetFields(args) {
...
async function handleExportResults(args) {
  ...
  for (const event of events) {
    const row = headers.map((h2) => {
      const val = String(event[h2] ?? "").replace(/"/g, '""');
...
// src/tools/listDashboards.ts
var listDashboardsSchema = {
...
async function handleListDashboards(args) {
  ...
  const entries = data?.entry ?? [];
  const dashboards = entries.map((e2) => ({
    name: e2.name,
    label: e2.content?.label || e2.name,
    isDashboard: e2.content?.isDashboard,
    isVisible: e2.content?.isVisible,
    updated: e2.updated
  }));
  return {
...
// src/tools/runDashboardPanel.ts
var runDashboardPanelSchema = {
...
async function handleRunDashboardPanel(args) {
...
// src/tools/getDatamodels.ts
var getDatamodelsSchema = {
  name: "get_datamodels",
...
async function handleGetDatamodels(args) {
  const { app, count = 50 } = args;
  const path = app ? `/servicesNS/-/${app}/datamodel/model` : "/services/datamodel/model";
  const data = await apiClient.get(path, { count: String(count) });
  const entries = data?.entry ?? [];
  const models = entries.map((e2) => ({
    name: e2.name,
    displayName: e2.content?.displayName || e2.name,
    description: e2.content?.description || "",
    accelerated: e2.content?.["acceleration.enabled"] === "1",
    app: e2.acl?.app
  }));
  return {
...
// src/tools/getDatamodelFields.ts
var getDatamodelFieldsSchema = {
...
async function handleGetDatamodelFields(args) {
  ...
  const objects = (modelDef.objects ?? []).map((obj) => ({
    name: obj.objectName,
    displayName: obj.displayName,
    parentName: obj.parentName,
    baseSearch: obj.baseSearch,
    fields: (obj.fields ?? []).map((f3) => ({
      fieldName: f3.fieldName,
      displayName: f3.displayName,
      type: f3.type,
      required: f3.required,
      multivalue: f3.multivalue
    })),
    constraints: (obj.constraints ?? []).map((c) => ({
      search: c.search,
      owner: c.owner
    }))
  }));
...
// src/tools/listReports.ts
var listReportsSchema = {
  name: "list_reports",
...
async function handleListReports(args) {
  ...
  const entries = data?.entry ?? [];
  const reports = entries.filter((e2) => !scheduledOnly || e2.content?.is_scheduled === "1").map((e2) => ({
    name: e2.name,
    search: e2.content?.search,
    cronSchedule: e2.content?.cron_schedule,
    isScheduled: e2.content?.is_scheduled === "1",
    nextScheduledTime: e2.content?.next_scheduled_time,
    lastRunTime: e2.content?.["dispatch.latest_time"],
    disabled: e2.content?.disabled === "1",
    actions: e2.content?.actions
  }));
  return {
...
// src/tools/getReportResults.ts
var getReportResultsSchema = {
...
async function handleGetReportResults(args) {
...
var server = new SplunkMCPServer();
