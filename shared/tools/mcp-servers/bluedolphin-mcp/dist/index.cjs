...
var require_client = __commonJS({
  "build/client.js"(exports2) {
    ...
    exports2.BlueDolphinClient = void 0;
    var BlueDolphinClient = class {
      config;
      lastRequestTime = 0;
      rateLimitDelay;
      maxRetries;
      ...
      async request(method, path, body) {
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
          try {
            const url = this.config.useOData ? `${this.odataBaseUrl}${path}` : `${this.restBaseUrl}${path}`;
            const headers = {
      ...
      async listObjects(params) {
        if (this.config.useOData) {
          ...
          const query = filters.length ? `?$filter=${filters.join(" and ")}&$top=${top}` : `?$top=${top}`;
    ...
    exports2.BlueDolphinClient = BlueDolphinClient;
    function sleep(ms) {
      ...
      try {
        ...
        switch (name) {
          case "bd_validate_connection":
            result = await client2.validateConnection();
            if (result.ok) {
          ...
          case "bd_search_objects":
            result = await client2.searchObjects({
              query: args.query,
            ...
            break;
          case "bd_get_object":
            result = await client2.getObject(args.object_id);
            break;
          case "bd_list_objects":
            result = await client2.listObjects({
              workspaceId: args.workspace_id,
              objectTypeId: args.object_type_id,
              limit: args.limit
            });
          ...
          case "bd_list_relations":
            result = await client2.listRelations(args.object_id);
            break;
          case "bd_list_workspaces":
            result = await client2.listWorkspaces();
            break;
          case "bd_list_object_types":
            result = await client2.listObjectTypes();
            break;
          default:
...
var server = new index_js_1.Server({ name: "bluedolphin-mcp", version: "0.1.0" }, { capabilities: { tools: {} } });
var client = new client_js_1.BlueDolphinClient({
