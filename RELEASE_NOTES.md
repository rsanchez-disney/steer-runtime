...
- **Harness MCP** — new MCP server with 7 tools for CI/CD pipeline management at disney.harness.io (list/trigger pipelines, executions, logs, services, environments)
- **Compass MCP delegation fix** — `@compass/*` added to sustainment orchestrator for direct access + new `network_diagnostics_agent` for DNS/cert/connectivity checks
- **DX Lodging Tools workspace** — hierarchical multi-team workspace (Rocket, Star Lord, Yondu) with custom alert analyst agent
- **Studio Ant-Man catalog** — 28 BAPPs fully populated with operational data, escalation contacts, Splunk queries, and troubleshooting guides
- **Release manager auto-commit** — `steer_release_manager_agent` now writes and commits RELEASE_NOTES/CHANGELOG on approval
- **Finder-services workspace_path** — switched from hardcoded path to `${WORKSPACE_ROOT}` for cross-platform portability
<!-- END LATEST -->
