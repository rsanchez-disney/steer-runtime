# Troubleshooting — DLR Capacity Managed Events
...
2. Check `wdpr_dlr_com_shared_api` for "cme-res-spa" errors
...
**Resolution:** See TMS DLR penalty troubleshooting. Route to `app-cadlr-galaxy` if penalty needs removal.
...
**Resolution:** Check MK tier blockout calendar. WAD for blocked pass types.
...
**Resolution:** Guest must cancel an existing reservation first. DLR Magic Key limits:
- Inspire Key: 6
- Believe Key: 4
- Enchant Key: 3
- Imagine Key: 2
...
**Resolution:** Follow ticket-upgrade-dlr flow.
...
- If Eligibility returns error → check entitlement source (eGalaxy DLR)
...
### Eligibility — By SWID (DLR)
```spl
search index=wdpr-ecommerce ecs_cluster="dlr-ecommerce-S0001477*" ecs_task_definition="cme-elig*" "{SWID}" earliest=-14d | sort -_time | head 30 | table _time, _raw
```

### Eligibility — By VID (DLR)
```spl
search index=wdpr-ecommerce ecs_cluster="dlr-ecommerce-S0001477*" ecs_task_definition="cme-elig*" "{VID}" earliest=-14d | sort -_time | head 30 | table _time, _raw
```

### Availability — By ConversationId (DLR)
```spl
search index=wdpr-ecommerce ecs_cluster="dlr-ecommerce-S0001477*" ecs_task_definition="cme-avail*" "{CONVERSATION_ID}" earliest=-7d | sort -_time | head 30
```

### Reservation — By ConversationId (DLR)
```spl
search index=wdpr-ecommerce ecs_cluster="dlr-ecommerce-S0001477*" ecs_task_definition="cme-res*" "{CONVERSATION_ID}" earliest=-7d | sort -_time | head 30
```

### Reservation — By SWID
```spl
search index=wdpr-ecommerce ecs_cluster="dlr-ecommerce-S0001477*" ecs_task_definition="cme-res*" "{SWID}" earliest=-7d | head 10
```
...
### Retrieval — By ConversationId (DLR)
```spl
search index=wdpr-ecommerce ecs_cluster="dlr-ecommerce-S0001477*" ecs_task_definition="cme-rtrvl*" "{CONVERSATION_ID}" earliest=-7d | sort -_time | head 30
```

### Retrieval — By SWID (DLR)
```spl
search index=wdpr-ecommerce ecs_cluster="dlr-ecommerce-S0001477*" ecs_task_definition="cme-rtrvl*" "{SWID}" earliest=-7d | sort -_time | head 30
```
...
### RES SPA — By SWID (DLR)
```spl
search index=wdpr_dlr_com_shared_api "cme-res-spa" "{SWID}" earliest=-7d | head 20
```
