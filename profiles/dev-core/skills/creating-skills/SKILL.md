...
description: >-
  Creates production-ready skills in the Anthropic open-standard format (SKILL.md + references/
  + scripts/) from workflows, prompts, or ideas. Use when the user wants to build a new skill
  or convert an existing prompt/workflow into a reusable skill folder.
---
...
- User has a draft skill and wants to iterate it to production quality.

## When NOT to use

- User wants to audit/evaluate an existing skill (use `auditing-skills` instead).
- User wants to design an agent configuration (use `designing-agents` instead).
